...
($+$)  :: Doc -> Doc -> Doc     -- Above; no dovetailing.
...
aboveNest _                   _ _ _ = error "Pretty.aboveNest: unhandled case"
\end{code}
...
sep1 _                   _ _ _  = error "Pretty.sep1: unhandled case"
...
fill1 _                   _ _ _  = error "Pretty.fill1: unhandled case"

fillNB :: Bool -> RDoc -> Int -> [Doc] -> Doc
...
best OneLineMode _ _ p
  ...
  where
    get Empty                = Empty
    get NoDoc                = NoDoc
    get (NilAbove p1)        = nilAbove_ (get p1)
    get (TextBeside s sl p1) = textBeside_ s sl (get p1)
    get (Nest _ p1)          = get p1             -- Elide nest
    get (p1 `Union` q)       = first (get p1) (get q)
    get  _                   = error "Pretty.best.get: unhandled case"
...
best _ w1 r p1
  ...
  where
    ...
    get _ _                   = error "Pretty.best.get: unhandled case"
    ...
    get1 w sl (p `Union` q)       = nicest1 w r sl (get1 w sl p) 
                                                   (get1 w sl q)
    get1 _ _  _                   = error "Pretty.best.get1: unhandled case"
...
fits _ _                   = error "Pretty.fits: unhandled case"
...
nonEmptySet  _                 = error "Pretty.nonEmptySet: unhandled case"
\end{code}
...
oneLiner  _                  = error "Pretty.oneLiner: unhandled case"
\end{code}
...
fullRender mode line_length ribbons_per_line txt end doc
  ...
  where 
    ...
    hacked_line_length = case mode of { ZigZagMode -> maxBound; _ -> line_length }
    ...
    let
        ...
        lay k (TextBeside s sl p)
            = case mode of
                    ...
                    _ -> lay1 k s sl p
        lay _ _            = error "Pretty.display.lay: unhandled case"
        ...
        lay2 _ Empty               = end
        lay2 _ _                   = error "Pretty.display.lay2: unhandled case"
    in
    lay 0 doc
    }}
...
easy_display nl_text txt end doc 
  ...
  where
    ...
    lay _                   _      = error "Pretty.easy_display.lay: unhandled case"
